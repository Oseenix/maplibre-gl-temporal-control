...
class Xr {
  ...
  calculateHeights() {
    const e = this.container.getBoundingClientRect().height, t = 32, r = 4 * this.colorSteps.length, i = e - this.titleDiv.offsetHeight - t - r, s = Math.max(Math.floor(i / this.colorSteps.length), 10), o = Math.ceil(this.colorSteps.length / (i / 30));
   ...
   */
  getColorSteps() {
    const t = this.propertySpec["fill-color"].default;
    if (t[0] !== "step")
      throw new Error("Only 'step' expressions are supported.");
    const i = [], [, , s, ...o] = t;
    i.push({ speed: 0, color: s });
    const a = this.options.max || 30;
    for (let l = 0; l < o.length; l += 2) {
      const c = o[l] * a, f = o[l + 1];
      i.push({ speed: c, color: f });
    }
    return i.reverse();
  }
  /**
